...
\end{equation} 
Note that the original orthogonal Procrustes problem is a special case of the problem (\ref{e2}). When there is no prior known column at all, i.e., $\mathbf{X}=\oldemptyset$, we have $p=0,q=n$ and the studied problem is reduced to the classical orthogonal Procrustes problem.
