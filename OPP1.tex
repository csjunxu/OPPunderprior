...
\end{equation}
and hence we have $\mathbf{U}=\mathbf{P}\mathbf{V}$ and equivalently $\mathbf{P}=\mathbf{U}\mathbf{V}^{\top}$. Note that we can also employ the property of symmertric matrix that every symmertric matrix can be diagonalized to obtain this results.
