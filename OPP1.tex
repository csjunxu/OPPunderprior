...
Since $\mathbf{P}$ is independent of $\textbf{Tr}(\Sigma)$, for which to be maximum, we need all the diagonal elements in $\Sigma$ to be nonnegative. Hence, once the $\mathbf{V}$ is given, the orientation of $\mathbf{U}$ must be chosen to retain the nonnegative diagonal elements in $\textbf{Tr}(\Sigma)$.
