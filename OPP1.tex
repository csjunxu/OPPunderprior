...
Now we proof the sufficiency condition. If $\mathbf{\hat{P}}=\mathbf{U}\mathbf{V}^{\top}$, then it is easy to check that $\mathbf{\hat{P}}^{\top}\mathbf{\hat{P}}=\mathbf{I}_{q\times q}$. To proof that $\mathbf{X}^{\top}\mathbf{\hat{P}}=\mathbf{0}_{p\times q}$, we can resort the Eq. (\ref{e5}) for help. Since $\mathbf{X}^{\top}\mathbf{X}=\mathbf{I}_{p\times p}$, by left multiplying both sides of Eq. (\ref{e5}) by $\mathbf{X}^{\top}$, we have  
\begin{equation}
