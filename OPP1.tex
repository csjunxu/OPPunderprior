...
\end{equation}
where $\mathbf{U}^{*}\in \mathbb{R}^{n\times q}$ and $\mathbf{V}^{*}\in \mathbb{R}^{q\times q}$ are arbitrarily orientated singular vectors of $\mathbf{U}$ and $\mathbf{V}$, respectively. The $\mathbf{K}_{u}$ and $\mathbf{K}_{v}$ are diagnonal matrices with $+1$ or $-1$ as diagonal elements in arbitrary distribution. If we fix the . $\mathbf{\Sigma}\in \mathbb{R}^{q\times q}$ are singular values in weak descending order, which means that $\mathbf{\Sigma}_{11}\ge\mathbf{\Sigma}_{22}\ge...\ge\mathbf{\Sigma}_{qq}\ge0$.
